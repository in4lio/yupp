...
It can also be useful for more general purposes. For example, it can
...
Source files for preprocessing are passed to **yupp** as command-line
...
Imported modules with their own `yupp` source cookie are not supported. The
transparent codec also requires normal `site` startup; `python -S`, stdin,
